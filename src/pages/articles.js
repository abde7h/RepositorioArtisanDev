...
const MovingImage = ({ title, img, link }) => {
  ...
  return (
    ...
    >
      <h2 className="capitalize text-xl font-semibold hover:cursor-pointer hover:underline">
        {title}
      </h2>
      <FramerImage
        style={{ x: x, y: y }}
...
const FeaturedArticle = ({ img, title, time, summary, link }) => {
  return (
    <li className="relative col-span-1 w-full p-4 bg-light border border-solid border-dark rounded-2xl dark:bg-dark dark:border-light">
      ...
      <Link href={link} target="_blank">
        <h2 className="capitalize text-2xl font-bold my-2 mt-4 hover:cursor-pointer hover:underline xs:text-lg">
          {title}
        </h2>
      </Link>
...
const Articles = () => {
  return (
    <>
      <Head>
        <title>ArtisanDev | Articles Page</title>
        <meta
          name="description"
          content="Explora mi blog de desarrollo web, donde comparto conocimientos sobre las últimas tendencias tecnológicas, consejos prácticos y análisis detallados. Aprende conmigo y mantente al día en la industria de la web."
        />
      </Head>
